export class Termo{
  ...
  constructor(){
	...
	verificadorDePalavra(palavraChutada: string): Cor[]{
		//V = verde, A = amarelo, N = nada
		this.arrayCor = [];
		for(let i = 0; i < 5; i++){
			if(palavraChutada[i] == this.palavraSorteada[i]){
				this.arrayCor.push(Cor.verde);
			}
			else if(this.palavraSorteada.includes(palavraChutada[i])){
				this.arrayCor.push(Cor.amarelo);
			}
			else {
				this.arrayCor.push(Cor.nada);
			}
		}
		return this.arrayCor;
	}

	verificarAcertou(palavraChutada: string): boolean{
		return this.palavraSorteada == palavraChutada;
	}
  ...
  sortearPalavra(){
    const arrayPalavras = ["ácido", "adiar", "ímpar", "algar", "amado", "amigo", "anexo", "anuir", "aonde", "apelo",
